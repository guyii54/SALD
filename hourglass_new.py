...
class HourglassModel():
    ...
    """
    def __init__(self,params, dataset = None, training = True, w_summary = True, modif = False, attention=False ,w_loss = False, name = 'tiny_hourglass',mode='hg'):
        """ Initializer
        Args:
        ...
        self.w_loss = w_loss
        self.params = params
        self.mode = mode    # 'skey': 4 channel input/  'hg': 3 channel input
        self.imgdim = 3 + self.params['outchannel']
    ...
    def get_train_hed_pic(self):
        # stack4, joint1
        tmp = self.output[1, 3, :,:,0]
        out = tf.expand_dims(tmp, axis=2)
    ...
    def get_output(self):
        ...
        for i in range(self.nStack):
            # self.output: TensorShape([Dimension(None), Dimension(4), Dimension(64), Dimension(64), Dimension(12)])
            tmp = self.output[1, i, :, :, :]
            # out put1: TensorShape([Dimension(64), Dimension(64), Dimension(12)])
            output = tf.reduce_sum(tmp, axis=2)
    ...
    def generate_model(self):
        ...
        with tf.device(self.gpu):
            with tf.name_scope('inputs'):
                if self.mode == 'skey':
                    self.img = tf.placeholder(dtype=tf.float32, shape=(None, 256, 256, self.imgdim), name='input_img')
                else:
        ...
        with tf.device(self.cpu):
            ...
            with tf.name_scope('lr'):
                self.lr = tf.train.exponential_decay(learning_rate=self.params['learning_rate'],
                                                     global_step=self.train_step,
                                                     decay_steps=self.params['decay_step'],
                                                     decay_rate=self.params['learning_rate_decay'],
                                                     staircase= True,
                                                     name= 'learning_rate')
            lrTime = time.time()
        ...
        with tf.device(self.cpu):
            with tf.name_scope('training'):
                ...
                output3 = output[2]
                output4 = output[3]
                head = self.get_train_hed_pic()
                img = tf.expand_dims(self.img[1, :, :, :], axis=0)
                gt = self.gtMaps[1,1,:,:,:]
                gt = tf.expand_dims(tf.reduce_sum(gt, axis=2), axis=0)
                gt = tf.expand_dims(gt, axis=3)
                tf.summary.image('input', img, collections=['train'])
                tf.summary.image('GT', gt, collections=['train'])
                tf.summary.image('out1',output1, collections=['train'])
                tf.summary.image('out2',output2, collections=['train'])
                tf.summary.image('out3',output3, collections=['train'])
                tf.summary.image('out4',output4, collections=['train'])
                tf.summary.image('head',head, collections=['train'])
                # scalar
    ...
    def _train(self,begin, nEpochs = 10, epochSize = 1000, saveStep = 500, validIter = 10):
        """
        """
        with tf.name_scope('Train'):
            self.generator = self.dataset.generate(self.params)
            self.valid_gen = self.dataset.generate(self.params)
            startTime = time.time()
            self.resume = {}
            self.resume['accur'] = []
            self.resume['loss'] = []
            self.resume['err'] = []
            for epoch in range(begin, nEpochs):
                ...
                for i in range(epochSize):
                    ...
                    sys.stdout.flush()
                    img_train, gt_train, weight_train, ske = next(self.generator)
                    if self.mode == 'skey':
                        img_train = np.concatenate((img_train,ske),axis=3)
                    ...
                    if i % saveStep == 0:
                        ...
                        if self.w_loss:
                            ...
                            _, c, lr= self.Session.run([self.train_rmsprop, self.loss,self.lr], feed_dict = {self.img : img_train, self.gtMaps: gt_train})
                    # # debug
                    # lr, step= self.Session.run([self.lr,self.train_step],
                ...
                with tf.name_scope('save'):
                    self.saver.save(self.Session, os.path.join(self.params['weight_saved'],str(self.name + '_' + str(epoch + 1))))
                self.resume['loss'].append(cost)
                # Validation Set
                accuracy_array = np.array([0.0]*len(self.joint_accur))
                for i in range(validIter):
                    img_valid, gt_valid, w_valid,ske = next(self.generator)
                    if self.mode == 'skey':
                        img_valid = np.concatenate((img_valid,ske),axis=3)
    ...
    def training_init(self, nEpochs = 10, epochSize = 1000, saveStep = 500, dataset = None, load = None,begin=0):
        ...
        #         self.saver.restore(self.Session, load)
            ...
            self._train(begin, nEpochs, epochSize, saveStep, validIter=10)
    ...
    def _graph_hourglass(self, inputs):
        ...
        with tf.name_scope('model'):
            with tf.name_scope('preprocessing'):
                # Input Dim : nbImages x 256 x 256 x 4 or 256 x 256 x 3
                pad1 = tf.pad(inputs, [[0,0],[2,2],[2,2],[0,0]], name='pad_1')
                # Dim pad1 : nbImages x 260 x 260 x 4
                # kernel size here is depending on inputs.shape[3]. [6, 6, inputs.shape[3], filters]
                ...
                if self.tiny:
                    # Dim r3: nbImages x 64 x 64 x 256
            ...
            if self.tiny:
                ...
                with tf.name_scope('stacks'):
                    with tf.name_scope('stage_0'):
                        hg[0] = self._hourglass(r3, self.nLow, self.nFeat, 'hourglass')
                        # drop[0] = tf.layers.dropout(hg[0], rate = self.dropout_rate, training = self.training, name = 'dropout')
                        drop[0] = keras.layers.Dropout(rate=self.dropout_rate, name='dropout', trainable=self.training)(hg[0])
    ...
    def _compute_err(self, u, v):
        ...
        return tf.divide(tf.sqrt(tf.square(tf.cast((u_x - v_x),tf.float32)) + tf.square(tf.cast((u_y - v_y),tf.float32))), tf.cast(91,tf.float32))
    ...
    def _accur(self, pred, gtMap, num_image):
        ...
        return tf.subtract(tf.cast(1,tf.float32), err/num_image)


    def pred(self, outputPath, test_num,load_path):
        '''
        prediction
        :param outputPath:
        :return:
        ...
        key_out = self.output
        pred_sigmoid = tf.nn.sigmoid(key_out[:, self.nStack - 1], name='sigmoid_final_prediction')
        pred_final = key_out[:, self.nStack - 1]
        joint_tensor = self._create_joint_tensor(key_out[0], name='joint_tensor')
        ...
        for i in range(1, test_num):
            percent = int(i/test_num) * 100
            num = np.int(20 * percent / 100)
            if (i % 100) == 0:
                # print('\rDeploying in Test Sets:{0}'.format('='*num), end='')
                print('deployed %d' % i)
            # heatmap
            img, gtmap, visi_weights, ske, name = next(generator)
            if self.mode == 'skey':
                # # -------test ske output------------
                # ske_r = cv2.imread(os.path.join(r'D:\Airplane Keypart\skey_data\HED\output_1218\ske',name))
                # ske_r = cv2.cvtColor(ske_r,cv2.COLOR_BGR2GRAY)
                # ske_r = np.expand_dims(ske_r,axis=0)
                # ske_r = np.expand_dims(ske_r,axis=3)
                img = np.concatenate((img, ske), axis=3)
            # if img.shape == (256, 256, 3):
            out, j = sess.run([pred_sigmoid, joint_tensor_final], feed_dict={self.img: img})
            # else:
            ...
            joints[name] = j
            pic_count += 1
        np.save(os.path.join(outputPath,'heatmaps.npy'),heatmaps)
        np.save(os.path.join(outputPath,'joints.npy'),joints)
        print('%d was inferenced' % pic_count)
